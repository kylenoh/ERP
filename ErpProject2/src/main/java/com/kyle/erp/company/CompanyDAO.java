...
public class CompanyDAO {
	...
	public void regCompany(Company company,HttpServletRequest req, HttpServletResponse res){
		...
		try {
			...
			String com_logo = mr.getFilesystemName("com_logo");
	...
	public void getCompany(HttpServletRequest req, HttpServletResponse res){
		Company company = ss.getMapper(CompanyMapper.class).getCompany();
		req.setAttribute("company", company);
	}
	public void updateCompany(Company company,HttpServletRequest req, HttpServletResponse res){
		MultipartRequest mr = null;
		String path = req.getSession().getServletContext().getRealPath("resources/file");
		try {
			mr = new MultipartRequest(req, path, 50 * 1024 * 1024, "utf-8", new DefaultFileRenamePolicy());
		...
		try {
			...
			if (ss.getMapper(CompanyMapper.class).updateCompany(company) == 1) {
			}
		} catch (Exception e) {
			e.printStackTrace();
			File f = new File(path + "/" + mr.getFilesystemName("com_logo"));
